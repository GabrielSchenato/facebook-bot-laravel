<template>
    <div>
        ...
        <div v-if="menu.menu_buttons.length > 0">
            <div v-for="menu_buttons in menu.menu_buttons" style="margin-bottom: 10px;"
                ...
                <div class="waves-effect waves-light child btn-large grey" v-for="child_menu_buttons in menu.menu_buttons"
                    v-bind:key="child_menu_buttons.id">
                    {{ child_menu_buttons.title }} <small>{{ child_menu_buttons.type | menus_types }} {{
                        child_menu_buttons.value }}</small>
                </div>
            </div>
        </div>
        ...
        <form @submit.prevent="create()" id="addButton" class="grey lighten-5">
            ...
            <div class="">
                <select v-model="newButton.parent_id" class="browser-default">
                    ...
                    <option v-for="menu_buttons in menu.menu_buttons" :value="menu_buttons.id" v-bind:key="menu_buttons.id">{{
                        menu_buttons.title }}</option>
                </select>
            </div>
            <hr>
    ...
    export default {
        ...
        computed: {
            menu() {
                ...
                return filtraMenuButtons(menu.menu_buttons) ? menu : { menu_buttons: [] }
            },
            filtraMenuButtons(menu_buttons) {

                menu_buttons.forEach(function (menu_button, index, object) {
                    if (!menu_button.parent_id == 0) {
                        object.splice(index, 1)

                        menu_buttons.forEach(function (child_menu_button, index, object) {
                            if (!menu_button.id === child_menu_button.parent_id) {
                                object.splice(index, 1)
                            }
                        })
                    }
